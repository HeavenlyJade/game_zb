...
function WeaponPlant:Init(positionIndex)
    ...
    -- 初始化子弹对象池
    self:InitBulletPool(bulletName)
end

function WeaponPlant:InitBulletPool(bulletName)
    local bulletConfigNode = MS.Config.GetCustomConfigNode(bulletName)
    local bulletData = require(bulletConfigNode.Data)
    local bulletModel = bulletConfigNode.Model

    for i = 1, self.bulletPoolCapacity do
        local bulletNode = bulletModel:Clone()
        local bullet = {
            Uid = MS.UidGenerator:GenerateUid(),
            bindActor = bulletNode,
            effectAssetID = bulletData.Attributes.hitEffect,
            soundAssetID = bulletData.Attributes.hitSound,
            enabled = false,
            spawner = self
        }

        bulletNode.Name = bulletName
        bulletNode.Parent = self.firePosition
        bulletNode.Position = self.firePosition.Position
        bulletNode.CollideGroupID = 4
...
function WeaponPlant:Fire(targetPosition)
    ...
    local bulletActor = self:GetIdleBullet()
    self:EnableBullet(bulletActor)
